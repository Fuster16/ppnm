...
//	double meanabs=0; 
//	for(int i=0;i<size;i++) meanabs += Abs(this[i]); // sums the absolute value of all indices?

//	if(meanabs==0) meanabs = 1; // if the vector isn't the 0 vector, set equal to 1?

//	meanabs /= size; // Finds the average of the absolute values?

//	double sum=0;

//	for(int i=0;i<size;i++)sum+=(this[i]/meanabs)*(this[i]/meanabs); // this[i]^2 * 1/(meanabs^2)

//	return meanabs*Sqrt(sum); // The meanabs are multiplied away again?!??!
//	}

public double norm(){ // making my own norm calculator
